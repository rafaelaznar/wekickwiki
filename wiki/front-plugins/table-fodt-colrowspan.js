...
WKW.registerPlugin(
  {
    ...
    version: "1.0.0",
    description:
      "Renders HTML <table> blocks with colspan and rowspan in the ODT export.",
    author: "WeKickWiki",
  ...
  },
  {
    "wkw.odt.htmlTable": function (current, html, ctx) {
      ...
      for (let r = 0; r < numRows; r++) {
        while (placement[r].length < numCols)
          placement[r].push({ covered: true });
      }
      ...
      for (const row of placement) {
        ...
        for (const cell of row) {
          if (cell.covered) {
            ...
            if (cell.colspan > 1)
              attrs += ' table:number-columns-spanned="' + cell.colspan + '"';
            if (cell.rowspan > 1)
              attrs += ' table:number-rows-spanned="' + cell.rowspan + '"';
            t +=
              '<table:table-cell table:style-name="' + cs + '"' + attrs + ">";
            t +=
              '<text:p text:style-name="' +
              ps +
              '">' +
              odtInline(cell.content) +
              "</text:p>";
            t += "</table:table-cell>";
          }
        }
