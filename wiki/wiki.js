...
window.WKW = (() => {
  ...
  return {
    ...
    registerPlugin(meta, handlers = {}) {
      if (!meta?.id) {
        console.warn("[WKW] Plugin missing id");
        return;
      }
      if (_plugins[meta.id]) {
        console.warn("[WKW] Already registered:", meta.id);
        return;
      }
    ...
    applyFilters(hook, value, ...args) {
      return (_filters[hook] ?? [])
        ...
        .reduce(
          (val, { fn, plugin }) =>
            _disabled.has(plugin) ? val : fn(val, ...args),
          value,
        );
    },
    /**
    ...
    setEnabled(id, val) {
      val ? _disabled.delete(id) : _disabled.add(id);
    },
    ...
    loadState(disabledArr) {
      ...
      if (Array.isArray(disabledArr))
        disabledArr.forEach((id) => _disabled.add(id));
    },
    /**
  ...
  while (i < text.length) {
    ...
    if (text[i] === "<") {
      const tag = text
        .slice(i)
        .match(
          /^<(\/?)(strong|b|em|i|code|s|del|strike|u|ins|mark|br)(\s[^>]*)?\/?>|^<a(\s[^>]*)?>|^<\/a>/i,
        );
      if (tag) {
        const full = tag[0];
    ...
    if (text.startsWith("**", i)) {
      ...
      if (j !== -1) {
        out +=
          '<text:span text:style-name="C_Bold">' +
          odtInline(text.slice(i + 2, j)) +
          "</text:span>";
        i = j + 2;
        continue;
      }
    ...
    if (text.startsWith("~~", i)) {
      ...
      if (j !== -1) {
        out +=
          '<text:span text:style-name="C_Strike">' +
          odtInline(text.slice(i + 2, j)) +
          "</text:span>";
        i = j + 2;
        continue;
    ...
    if (text[i] === "*" && !text.startsWith("**", i)) {
      ...
      if (j !== -1 && !text.startsWith("**", j)) {
        out +=
          '<text:span text:style-name="C_Italic">' +
          odtInline(text.slice(i + 1, j)) +
          "</text:span>";
        i = j + 1;
        continue;
      }
    ...
    if (text[i] === "`") {
      ...
      if (j !== -1) {
        out +=
          '<text:span text:style-name="C_Code">' +
          odtXmlEsc(text.slice(i + 1, j)) +
          "</text:span>";
        i = j + 1;
        continue;
      }
...
function odtBody(md, inQuote) {
    ...
    while (j < rawLines.length) {
      ...
      if (/^>/.test(rawLines[j])) {
        ...
        while (j < rawLines.length && /^>/.test(rawLines[j])) {
          qLines.push(rawLines[j].replace(/^>\s?/, ""));
          j++;
        }
      ...
      if (
        ...
        while (
          ...
          rows.push(splitCells(rawLines[k]));
          k++;
        }
  ...
  function odtTable(headers, rows) {
    ...
    let t =
      ...
      odtXmlEsc(tname) +
      '" table:style-name="T_Table">';
    for (let c = 0; c < cols; c++)
      t += '<table:table-column table:style-name="T_Col"/>';
    // header row
    t += "<table:table-header-rows><table:table-row>";
    for (const h of headers)
      t +=
        '<table:table-cell table:style-name="T_CellH" office:value-type="string"><text:p text:style-name="P_TH">' +
        odtInline(h) +
    ...
    for (const row of rows) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        t +=
          '<table:table-cell table:style-name="T_Cell" office:value-type="string"><text:p text:style-name="P_TD">' +
          odtInline(cell) +
          "</text:p></table:table-cell>";
      }
      t += "</table:table-row>";
  ...
  const lines = tokens.map((t) =>
    t.type === "line"
      ? t.text
      : t.type === "table"
        ? "\x00TABLE\x00" + JSON.stringify({ h: t.headers, r: t.rows })
        : t.type === "htmltable"
          ? "\x00HTMLTABLE\x00" + t.html
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("\x00TABLE\x00")) {
      ...
      out += WKW.applyFilters("wkw.odt.htmlTable", "", line.slice(11), {
        odtXmlEsc,
        odtInline,
        nextTableName: () => "Tbl" + ++_odtTableCounter,
      });
      ...
      if (hd) {
        ...
        if (!inList || lstType !== "ul") {
          if (inList) out += "</text:list>";
          out += '<text:list text:style-name="LS_Bullet">';
          inList = true;
          lstType = "ul";
        }
        out +=
        ...
        if (!inList || lstType !== "ol") {
          if (inList) out += "</text:list>";
          out += '<text:list text:style-name="LS_Number">';
          inList = true;
          lstType = "ol";
        }
        out +=
...
document
  ...
  .addEventListener("change", async function () {
    ...
    try {
      const res = await apiFetch("?action=restore", {
        method: "POST",
        body: fd,
      });
      const data = await res.json().catch(() => ({}));
      if (res && res.ok) {
        showToast(
          "Backup restored — " + data.pages + " page(s) recovered.",
          "success",
          5000,
        );
        load(currentPage);
...
async function toggleSettingsPanel() {
  ...
  document.getElementById("settings-save-status").textContent = "";
  document.getElementById("settings-code-line-numbers").checked =
    !!settings.codeLineNumbers;
  document.getElementById("settings-guest-odt-download").checked =
    settings.guestOdtDownload !== false;
  document.getElementById("settings-guest-toc").checked =
    settings.guestToc !== false;
  document.getElementById("settings-guest-index").checked =
    settings.guestIndex !== false;
}
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const codeLineNumbers = document.getElementById(
      "settings-code-line-numbers",
    ).checked;
    const guestOdtDownload = document.getElementById(
      "settings-guest-odt-download",
    ).checked;
...
function navigate(page, replace) {
  const url =
    "wiki.php" + (page === "index" ? "" : "?p=" + encodeURIComponent(page));
  replace
...
function showWiki() {
  ...
  document.getElementById("toc-btn").style.display =
    isAdmin || (isGuest && window.WKW_GUEST_TOC) ? "" : "none";
  document.getElementById("edit-btn").style.display = isAdmin ? "" : "none";
  document.getElementById("index-btn").style.display =
    isAdmin || (isGuest && window.WKW_GUEST_INDEX) ? "" : "none";
  ...
  document.getElementById("change-pass-btn").style.display = isGuest
    ? ""
    : "none";
...
async function load(page) {
  ...
  if (res.status === 404 && isAdmin) {
    ...
    document.getElementById("content").innerHTML =
      ...
      '<button class="btn btn-primary" title="Create page" aria-label="Create page" onclick="createPage()">' +
      ICON_PLUS +
      "</button>";
  } else {
    document.getElementById("delete-btn").style.display = isAdmin ? "" : "none";
    const canDownloadOdt =
      isAdmin || (getRole() === "guest" && window.WKW_GUEST_ODT_DOWNLOAD);
    document.getElementById("odt-btn").style.display = canDownloadOdt
      ? ""
      : "none";
  ...
  document.querySelectorAll("#content a[href]").forEach((a) => {
    ...
    if (
      ...
      const base = page.includes("/")
        ? page.slice(0, page.lastIndexOf("/") + 1)
        : "";
...
function toggleToc() {
  ...
  if (!headings.length) {
    document.getElementById("toc-list").innerHTML =
      '<p style="color:#888;font-size:.85rem;padding:.25rem 0">No headings on this page.</p>';
    return;
  }
  let html = "<ul>";
  headings.forEach((h) => {
    ...
    html +=
      ...
      id +
      "').scrollIntoView({behavior:'smooth'});closeToc();return false;\">" +
      (h.dataset.label || h.textContent) +
      "</a></li>";
  });
  html += "</ul>";
  ...
  function renderTree(node, prefix) {
    ...
    for (const key of keys) {
      ...
      if (isPage || isAdmin) {
        const badge =
          !isPage && isAdmin
            ? ' <span class="index-no-page" title="No page file for this directory">\u2205</span>'
            : "";
        html +=
          '<a href="' +
          (path === "index"
            ? "wiki.php"
            : "wiki.php?p=" + encodeURIComponent(path)) +
          '" onclick="navigate(\'' +
          path +
          "');toggleIndex();return false;\">" +
          key +
          badge +
          "</a>";
      } else {
...
async function save() {
  ...
  if (res && res.ok) {
    ...
    const canDownloadOdt =
      getRole() === "admin" ||
      (getRole() === "guest" && window.WKW_GUEST_ODT_DOWNLOAD);
    document.getElementById("odt-btn").style.display = canDownloadOdt
      ? ""
      : "none";
    showToast(
      wasNew
        ? "Page \u201c" + currentPage + "\u201d created."
        : "Page \u201c" + currentPage + "\u201d saved.",
    );
  } else {
    const data = await res.json().catch(() => ({}));
    status.textContent = "";
    showToast(
      data.error || "Could not save the page. Please try again.",
      "error",
    );
  }
}
...
function togglePluginsPanel() {
  ...
  document.getElementById("plugins-overlay").style.display = _pluginsOpen
    ? "block"
    : "none";
  document.getElementById("plugins-panel").style.display = _pluginsOpen
    ? "block"
    : "none";
