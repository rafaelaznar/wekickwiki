<?php
// ═══════════════════════════════════════════════════════════════════════════
// wiki/wiki-api.php — Wiki-specific helper functions and API endpoint handlers.
//
// Provides:
...
function load_settings(): array
{
  ...
  $theme           = (isset($data['theme']) && is_string($data['theme']) && preg_match('/^[a-zA-Z0-9_\-]+\.css$/', $data['theme']) && is_file(__DIR__ . '/templates-wiki/' . $data['theme'])) ? $data['theme'] : $defaults['theme'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_GET['action'] ?? '') === 'save-settings') {
  ...
  if (!is_file(__DIR__ . '/templates-wiki/' . $theme)) {
