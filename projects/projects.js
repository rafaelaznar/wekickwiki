...
function ptRoute() {
  const role = getRole();
  document.getElementById("pt-user-badge").textContent =
    getUser() + " (" + role + ")";
  if (role === "admin") {
...
function ptPopulateProjectSelector(projects) {
  ...
  document.getElementById("pt-project-bar").style.display = projects.length
    ? ""
    : "none";
}
...
function ptUpdateProjectDates() {
  ...
  if (proj && (proj.start_date || proj.end_date)) {
    el.textContent = [proj.start_date, proj.end_date]
      .filter(Boolean)
      .join(" → ");
  } else {
    el.textContent = "";
...
async function ptLoadAdminData() {
  try {
    ...
    document.getElementById("pt-project-bar").style.display = _ptProjects.length
      ? ""
      : "none";
  } catch (e) {
...
async function ptLoadTasks(projectId) {
  ...
  try {
    const res = await apiFetch(
      "projects.php?action=get-all-tasks&project_id=" + projectId,
    );
    ...
    if (activeTab) {
      ...
      if (activeTab.dataset.tab === "burndown")
        ptRenderBurndown("burndown-wrap");
    }
  } catch (e) {
    ptToast(e.message, "error");
  }
...
function ptRenderProjectsTable() {
  ...
  if (!_ptProjects.length) {
    wrap.innerHTML =
      '<p class="pt-empty-msg">No projects yet. Create your first project.</p>';
    return;
  }
  let html =
...
function ptOpenProjectModal(id) {
  ...
  document.getElementById("project-modal-title").textContent = id
    ? "Edit Project"
    : "New Project";
...
async function ptSubmitProject() {
  ...
  try {
    const res = await apiFetch("projects.php?action=save-project", {
      method: "POST",
...
function ptConfirmDeleteProject(id) {
  ...
  ptShowConfirm(
    ...
    async () => {
      try {
        const res = await apiFetch("projects.php?action=delete-project", {
          method: "POST",
        ...
        if (_ptCurrentProjId === id) {
          _ptCurrentProjId = null;
          _ptTasks = [];
        }
        await ptLoadAdminData();
      } catch (e) {
...
function ptRenderTasksTable(tasks) {
  ...
  function depth(t) {
    ...
    while (
      cur.parent_id !== null &&
      cur.parent_id !== undefined &&
      allMap[cur.parent_id]
    ) {
      d++;
  ...
  let html =
    '<table class="pt-table pt-tasks-table"><thead><tr>' +
  ...
  const children = tasks
    .filter(
      (t) =>
        ...
        (parentId === null &&
          (t.parent_id === null || t.parent_id === undefined)),
    )
    .sort((a, b) => a.id - b.id);
  let result = [];
  children.forEach((c) => {
...
function ptApplyTaskFilters() {
  ...
  const searchF = document
    .getElementById("tf-search")
    .value.trim()
    .toLowerCase();
  ...
  _ptFilteredTasks = _ptTasks.filter((t) => {
    ...
    if (
      searchF &&
      !t.name.toLowerCase().includes(searchF) &&
...
function ptOpenTaskModal(taskId, parentId) {
  ...
  document.getElementById("task-modal-title").textContent = taskId
    ? "Edit Task"
    : parentId
      ? "New Subtask"
      : "New Task";
...
async function ptSubmitTask() {
  ...
  const assignees = [];
  document
    .querySelectorAll("#tm-assignees-wrap input[type=checkbox]:checked")
    .forEach((cb) => {
  ...
  const body = {
    ...
    integration_branch: document
      .getElementById("tm-integration-branch")
      .value.trim(),
    done: document.getElementById("tm-done").checked,
  };

  try {
    const res = await apiFetch("projects.php?action=save-task", {
      method: "POST",
      body: JSON.stringify(body),
    });
    if (!res.ok) {
...
function ptConfirmDeleteTask(id) {
  ...
  const extra =
    childCount > 0 ? ` This will also delete ${childCount} subtask(s).` : "";
  ptShowConfirm(
    `Delete task "${t ? t.name : id}"?${extra} This cannot be undone.`,
    async () => {
      try {
        const res = await apiFetch("projects.php?action=delete-task", {
          method: "POST",
          body: JSON.stringify({ id }),
        });
        if (!res.ok) {
...
function ptShowTaskDetail(taskId) {
  ...
  const parent = t.parent_id
    ? _ptTasks.find((x) => x.id === t.parent_id)
    : null;
...
function ptRenderBurndown(wrapId) {
  ...
  if (!proj.start_date || !proj.end_date) {
    wrap.innerHTML =
      '<p class="pt-empty-msg">Set start and end dates on the project to display the burndown chart.</p>';
    return;
  }
  ...
  if (totalPoints === 0) {
    wrap.innerHTML =
      '<p class="pt-empty-msg">No story points assigned. Add points to tasks to see the burndown chart.</p>';
    return;
  }
  ...
  // Ideal line points
  const idealX1 = PAD.left,
    idealY1 = toY(totalPoints);
  const idealX2 = PAD.left + CW,
    idealY2 = toY(0);

  // Actual polyline points
  const actualPts = actualDays
    .map((d) => `${toX(d.date).toFixed(1)},${toY(d.pts).toFixed(1)}`)
    .join(" ");
  ...
  const currentPts = actualDays.length
    ? actualDays[actualDays.length - 1].pts
    : totalPoints;
  const pctDone =
    totalPoints > 0
      ? Math.round(((totalPoints - currentPts) / totalPoints) * 100)
      : 0;
  ...
  ${
    showToday
      ? `<line x1="${todayX}" y1="${PAD.top}" x2="${todayX}" y2="${PAD.top + CH}" stroke="#e05" stroke-width="1.5" stroke-dasharray="3,3" opacity="0.7"/>
...
function ptOpenStatusModal(key) {
  ...
  document.getElementById("status-modal-title").textContent = key
    ? "Rename Status"
    : "New Status";
...
async function ptSubmitStatus() {
  ...
  try {
    ...
    const res = await apiFetch("projects.php?action=save-status", {
      method: "POST",
      body: JSON.stringify(body),
    });
    if (!res.ok) {
...
function ptConfirmDeleteStatus(key) {
  ...
  ptShowConfirm(
    ...
    async () => {
      try {
        const res = await apiFetch("projects.php?action=delete-status", {
          method: "POST",
...
async function ptApplyTheme(theme) {
  document.getElementById("pt-theme-link").href =
    "templates-projects/" + encodeURIComponent(theme);
  document.querySelectorAll(".pt-theme-option").forEach((b) => {
    b.classList.toggle(
      "active",
      b.textContent.trim() + ".css" === theme ||
        b.textContent.trim() === theme.replace(".css", ""),
    );
  });
  try {
    const res = await apiFetch("projects.php?action=save-projects-theme", {
      method: "POST",
...
function ptRenderUserMyTasks(tasks) {
  ...
  if (!tasks.length) {
    wrap.innerHTML =
      '<p class="pt-empty-msg">No tasks assigned to you in this project.</p>';
    return;
  }
  let html = "";
  ptStatusOrder().forEach((status) => {
