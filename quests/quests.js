...
function qsShowApp() {
  ...
  document.getElementById("qs-user-badge").textContent =
    getUser() + " (" + getRole() + ")";
}
...
async function qsLoadQueries() {
  ...
  try {
    ...
    const allLabels = [
      ...new Set(_allQueriesCache.flatMap((q) => q.labels || [])),
    ].sort();
...
function qsApplyQueryFilters() {
  const search = (document.getElementById("qf-search")?.value ?? "")
    .toLowerCase()
    .trim();
  ...
  const filtered = _allQueriesCache.filter((q) => {
    ...
    if (label && !(q.labels || []).some((l) => l.toLowerCase().includes(label)))
      return false;
    return true;
  });
  ...
  if (countEl) {
    ...
    countEl.textContent = active
      ? `${filtered.length} / ${_allQueriesCache.length}`
      : "";
  }
...
function qsRenderQueriesTable(queries) {
  ...
  if (!_allQueriesCache.length) {
    wrap.innerHTML =
      '<p class="qs-empty">No questions yet. Click “Add question” to create one.</p>';
    return;
  }
  if (!queries.length) {
    wrap.innerHTML =
      '<p class="qs-empty">No questions match the current filters.</p>';
    return;
  }
...
function qsOpenQueryModal(queryData = null) {
  ...
  document.getElementById("query-modal-title").textContent = queryData
    ? "Edit question"
    : "Add question";
  document.getElementById("qm-type").value = queryData
    ? queryData.type
    : "multiple_choice";
  document.getElementById("qm-query").value = queryData ? queryData.query : "";
  document.getElementById("qm-labels").value = queryData
    ? (queryData.labels || []).join(", ")
    : "";
...
async function qsEditQuery(id) {
  ...
  const res = await apiFetch("quests.php?action=get-queries");
  if (!res.ok) {
    qsToast("Failed to load question", "error");
    return;
  }
  const queries = await res.json();
  const q = queries.find((x) => x.id === id);
  if (!q) {
    qsToast("Question not found", "error");
    return;
  }
  qsOpenQueryModal(q);
}
...
function qsRenderQueryTypeFields(data = null) {
  ...
  if (type === "multiple_choice") {
    ...
    const opts = data?.options || {};
    const pairs = Object.entries(opts).length
      ? Object.entries(opts)
      : [["", ""]];
...
async function qsSaveQuery() {
  ...
  const labels = document
    .getElementById("qm-labels")
    .value.split(",")
    .map((s) => s.trim())
    .filter(Boolean);

  if (!query) {
    qsToast("Question text is required", "error");
    return;
  }
  ...
  if (type === "multiple_choice") {
    const opts = [...document.querySelectorAll(".mc-opt-input")].map((i) =>
      i.value.trim(),
    );
    const correct =
      document.querySelector('input[name="mc-correct"]:checked')?.value ?? "0";
    body.options = opts;
    body.answer = correct;
  } else if (type === "binary") {
    body.answer = document.getElementById("binary-answer").value;
  } else if (type === "gap_filling") {
    body.options = [...document.querySelectorAll(".gf-opt-input")]
      .map((i) => i.value.trim())
      .filter(Boolean);
  } else if (type === "matching") {
    const opts = {};
...
function qsRenderQuestsAdminTable(quests) {
  ...
  if (!quests.length) {
    wrap.innerHTML =
      '<p class="qs-empty">No quests match the selected filters.</p>';
    return;
  }
  ...
  for (const q of quests) {
    ...
    const allowedHtml = allowedVal.includes("all")
      ...
      : allowedVal
          .map((u) => `<span class="badge badge-label">${esc(u)}</span>`)
          .join(" ");
...
function qsApplyQuestFilters() {
  ...
  const filtered = _questsAllItems
    .filter((q) => {
      ...
      if (
        avgScoreFilter === "lt5" &&
        !(Number.isFinite(avgScore) && avgScore < 5)
      )
        return false;
      if (
        avgScoreFilter === "ge5" &&
  ...
  if (countEl)
    countEl.textContent = `${filtered.length} quest${filtered.length !== 1 ? "s" : ""}`;
...
async function qsOpenQuestModal(questData = null) {
  ...
  document.getElementById("quest-modal-title").textContent = questData
    ? "Edit quest"
    : "Add quest";
  document.getElementById("qst-name").value = questData?.name ?? "";
  document.getElementById("qst-date").value =
    questData?.date ?? new Date().toISOString().slice(0, 10);
  ...
  document.getElementById("qst-revisable").value = questData?.revisable
    ? "1"
    : "0";
...
async function qsEditQuest(id) {
  const res = await apiFetch("quests.php?action=get-quests-admin");
  if (!res.ok) {
    qsToast("Failed to load quest", "error");
    return;
  }
  const quests = await res.json();
  const q = quests.find((x) => x.id === id);
  if (!q) {
    qsToast("Quest not found", "error");
    return;
  }
  qsOpenQuestModal(q);
}
...
async function qsSaveQuest() {
  ...
  if (!name) {
    qsToast("Quest name is required", "error");
    return;
  }

  const groups = [];
  document
    .querySelectorAll("#qst-label-groups .qs-label-group")
    .forEach((div) => {
      const inputs = div.querySelectorAll("input");
      const labels = inputs[0].value
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
  ...
  // Read allowed field
  const allowedMode = document.querySelector(
    'input[name="qst-allowed-mode"]:checked',
  )?.value;
  let allowed;
  if (allowedMode === "specific") {
    allowed = [
      ...document.querySelectorAll('input[name="qst-user-cb"]:checked'),
    ].map((cb) => cb.value);
...
function qsConfirmDeleteQuest(id) {
  _deleteCallback = async () => {
    ...
    qsCloseDeleteModal();
    if (res.ok) {
      qsToast("Quest deleted");
      qsLoadQuestsAdmin();
    } else qsToast(data.error || "Delete failed", "error");
  };
  document.getElementById("delete-modal-title").textContent = "Delete quest";
  document.getElementById("delete-modal-msg").textContent =
    `Delete quest #${id}? All associated data will remain in the attempts file.`;
  document.getElementById("delete-modal-confirm").onclick = _deleteCallback;
  document.getElementById("delete-modal-overlay").classList.add("open");
}
...
async function qsAdminReviewAttempt(attemptId) {
  const res = await apiFetch(
    `quests.php?action=review-attempt&id=${attemptId}`,
  );
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    qsToast(data.error || "Failed to load review", "error");
    return;
  }
...
function qsToggleAllowedUsers(prefix) {
  const mode = document.querySelector(
    `input[name="${prefix}-allowed-mode"]:checked`,
  )?.value;
...
async function qsOpenAccessModal(id, currentAllowed) {
  ...
  document.getElementById("access-modal-title").textContent =
    "Manage access — quest #" + id;
...
async function qsSaveAccess() {
  const mode = document.querySelector(
    'input[name="acc-allowed-mode"]:checked',
  )?.value;
  let allowed;
  if (mode === "specific") {
    allowed = [
      ...document.querySelectorAll('input[name="acc-user-cb"]:checked'),
    ].map((cb) => cb.value);
    if (!allowed.length) allowed = ["all"];
  } else {
    allowed = ["all"];
...
document
  .getElementById("access-modal-overlay")
  .addEventListener("click", (e) => {
    if (e.target === document.getElementById("access-modal-overlay"))
      qsCloseAccessModal();
  });
...
function qsPopulateResultsFilters(items) {
  ...
  const quests = [
    ...new Set(items.map((a) => String(a.quest_name ?? "")).filter(Boolean)),
  ].sort((a, b) => a.localeCompare(b));
  const users = [
    ...new Set(items.map((a) => String(a.username ?? "")).filter(Boolean)),
  ].sort((a, b) => a.localeCompare(b));
  const months = [
    ...new Set(
      items
...
function qsApplyResultsFilters() {
  ...
  const filtered = _resultsAllItems
    .filter((a) => {
  ...
  if (!filtered.length) {
    wrap.innerHTML =
      '<p class="qs-empty">No results match the selected filters.</p>';
    return;
  }
  ...
  for (const a of filtered) {
    if (!groupMap.has(a.quest_id))
      groupMap.set(a.quest_id, { quest_name: a.quest_name, attempts: [] });
  ...
  for (const group of groups) {
    ...
    hdr.innerHTML =
      `<strong class="qs-results-quest-name">${esc(group.quest_name)}</strong>` +
...
async function qsLoadUserHome() {
  ...
  // Open quests
  {
    ...
    try {
      ...
      if (!quests.length) {
        wrap.innerHTML =
          '<p class="qs-empty">No open quests available for you right now.</p>';
      } else {
  ...
  // My attempts
  {
    ...
    try {
      ...
      if (!items.length) {
        wrap.innerHTML =
          '<p class="qs-empty">You haven\'t completed any quests yet.</p>';
      } else {
...
function qsUserShowView(view) {
  document.getElementById("user-quest-list").style.display =
    view === "quest-list" ? "" : "none";
  document.getElementById("user-wizard").style.display =
    view === "wizard" ? "" : "none";
  document.getElementById("user-review").style.display =
    view === "review" ? "" : "none";
  document.getElementById("user-result").style.display =
    view === "result" ? "" : "none";
}
...
async function qsStartQuest(questId) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    qsToast(data.error || "Failed to start quest", "error");
    return;
  }
...
function qsRenderAnswerArea(q, savedAnswer, readonly) {
  ...
  if (q.type === "multiple_choice") {
    ...
    const savedPairs =
      typeof savedAnswer === "object" && savedAnswer ? savedAnswer : {};
    q.keys.forEach((key) => {
      const li = document.createElement("li");
...
async function qsSubmitWizard() {
  if (!confirm("Submit this quest? You cannot change your answers afterwards."))
    return;
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    qsToast(data.error || "Submit failed", "error");
    return;
  }
...
function qsRenderReviewStep() {
  ...
  if (q.type === "multiple_choice") {
    ...
    isCorrect = (q.options || [])
      .map((s) => s.toLowerCase())
      .includes((userAns || "").toLowerCase().trim());
  } else if (q.type === "matching") {
    isCorrect = Object.entries(q.options || {}).every(
      ([k, v]) => (userAns || {})[k] === v,
    );
  }
...
function qsRenderReviewAnswerArea(q) {
  ...
  if (q.type === "multiple_choice") {
    ...
    if (String(userAns) !== correctIdx) {
      ...
      hint.textContent =
        "✔ Correct answer: " + (q.options[parseInt(correctIdx)] || "—");
    ...
    const input = document.createElement("input");
    const isOk = (q.options || [])
      .map((s) => s.toLowerCase())
      .includes((userAns || "").toLowerCase().trim());
    input.type = "text";
    input.className =
      "qs-gap-input " + (userAns ? (isOk ? "correct" : "wrong") : "");
    input.value = userAns ?? "";
    input.disabled = true;
    wrap.appendChild(input);
    if (!isOk) {
    ...
    Object.entries(correctPairs).forEach(([key, correctVal]) => {
      ...
      const sel = document.createElement("select");
      sel.className =
        "qs-matching-select " + (userVal ? (isOk ? "correct" : "wrong") : "");
      sel.disabled = true;
      allVals.forEach((val) => {
        const opt = document.createElement("option");
        opt.value = val;
...
function qsReviewNext() {
  if (_reviewStep < _reviewData.questions.length - 1) {
    _reviewStep++;
    qsRenderReviewStep();
  }
}
...
document
  .getElementById("qs-theme-select")
  ?.addEventListener("change", (e) => qsPreviewTheme(e.target.value));
...
async function qsSaveTheme() {
  ...
  if (!theme) return;
  const res = await apiFetch("quests.php?action=save-quest-theme", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
